...
`topic_embeddings` stores offline topic vectors generated by `scripts/embed_topics.py`. `user_profile_embeddings` stores aggregate user vectors built from selected topics and interaction feedback.
