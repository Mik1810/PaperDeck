...
`topic_embeddings` stores offline topic vectors generated by `scripts/embed_topics.py`. The first remote BGE-small smoke run has written 2 topic vectors and 1 paper vector, all with 384 dimensions. `user_profile_embeddings` stores aggregate user vectors built from selected topics and interaction feedback.
