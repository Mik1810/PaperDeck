...
4. Treat a direct `ok` / `vai` / `fallo` / `procedi` / `approvato` immediately
   after the plan as approval. The user may combine approval with an explicit
   low-risk same-issue extension (for example `ok vai, aggiorna anche il changelog`
   or `vai, bumpa anche la versione`); when intent is unambiguous, acknowledge the
   added surfaces and proceed without a redundant approval round. Questions,
   ambiguous/partial approval, or extensions that introduce a new migration,
   Production/hosted mutation, destructive action, privilege boundary, material
   architecture decision, or materially different transaction/locking risk still
   require the appropriate revised/consequential approval before that work begins.
...
6. Keep the implementation scoped to the approved issue plan and its explicit
   mutation surfaces. Treat those surfaces as a boundary against agent-initiated
   drift; a clear user instruction may directly extend a non-material same-issue
   surface without another plan round. Otherwise, before the first edit outside
   the surfaces, stop and request approval for a revised plan. If validation
   exposes a new runtime or product defect outside the approved goal, stop before
   fixing it even if the fix is needed to make the current issue green, unless
   the user explicitly adds that defect and no material-risk gate is crossed. New
   migrations, hosted
...
- Use `scripts/pd-run` for noisy commands. It keeps the full transcript on disk and returns only a bounded summary/diagnostic slice to the model.
- Never bypass a compact `pd-run` failure by dumping the raw log; use `scripts/pd-log <log> [pattern]` for one focused slice.
- For ordinary commands expected to finish within roughly a minute, use about 30 seconds for the initial tool yield and about 30 seconds for a necessary follow-up wait. Do not poll every 1–5 seconds.
- For clearly non-interactive local work likely to exceed roughly a minute (large container/browser pulls, long E2E/integration suites, scale benchmarks, expensive builds), prefer `scripts/pd-bg-run --label <label> -- <command>` and stop at `LOCAL_JOB_STATUS: WAITING` rather than spending repeated model turns waiting. On a later follow-up, call `scripts/pd-bg-status <job-id>` once; if still running, report and stop again.
- Do not repeatedly tail a background job. Inspect its compact completion output first, then use `pd-log` only if a targeted error slice is needed.
...
- After code delivery, a substantial same-issue Production/hosted/operational phase should normally use `scripts/pd-handoff <issue> <next-phase> [--pr N]` and continue in a fresh task with `scripts/pd-resume <issue> [phase]`. Keep the handoff concise: completed state, validation, pending work, hosted state, and approvals still required; never include secrets or raw logs.
- Do not force a new task for ordinary same-issue completion such as one post-CI snapshot/merge, review feedback, branch cleanup, or a small documentation follow-up while context remains compact. The user may explicitly choose to continue a substantial phase in the same task despite the context cost; consequential approval gates still apply.
- For same-issue follow-ups kept in the current task, reuse the established context; do not reread `SKILL.md`, rerun `context.sh`, or reread `PROJECT_STATE.md` unless repository state changed enough to require re-establishing context.
